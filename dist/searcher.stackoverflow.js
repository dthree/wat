...
var stackoverflow = {
  ...
  getQuestion: function getQuestion(questionId, callback) {
    callback = callback || {};
    var url = 'http://api.stackexchange.com/2.2/questions/' + questionId + '?order=desc&sort=activity&site=stackoverflow&filter=!)Ehu.SDh9PeCcJmhDxT60pU1mT_mgvdo9d3mN8WYbPzQzO6Te';
    util.fetchRemote({
      url: url,
      gzip: true
  ...
  formatAnswer: function formatAnswer(answ, margin) {
    ...
    var formatted = cosmetician.tab(cosmetician.markdownToTerminal(markdown, { lineWidth: process.stdout.columns - margin - 2 }), margin - 2);
