...
module.exports = function (vorpal) {
  ...
  Step.prototype.expect = function (evt, fn) {
    ...
    this._listener = {
      event: event,
      fn: function (e) {
        fn.call(vorpal, e, function (valid) {
          if (valid) {
  ...
  Step.prototype.log = function (str) {
    str = chalk.blue(str);
    let leftWidth = String(this._step).length + 4;
    let mainWidth = process.stdout.columns - leftWidth - 2;
    let wrapped = wrap(str, mainWidth);
  ...
  tour.step()
    .begin('Press [enter].')
    .end('Great!')
    .expect("submit", function (data, cb) {
      cb(true);
    });

  tour.step()
    .begin('Now, let\'s enter "foo".')
    .expect("command", function (data, cb) {
      let valid = (data.command === 'foo');
      if (!valid) {
        this.log('\n  Err.. wrong command.');
      }
      cb(valid);
