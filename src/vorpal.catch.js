  ...
  vorpal
    ...
    .action(function(args, cb){
      ...
      function execPath(pathObj) {
        let fullPath = util.command.buildExtension(pathObj.path, pathObj.index, args.options);
      ...
      if (path.exists === false) {
        if (path.suggestions) {
          ...
          let results = parent.clerk.search(args.commands.join(' '));

          if (results.length === 1 && results[0].points > 0) {

            self.log(chalk.yellow(`\n  Showing results for "${results[0].command}":`));
            let path = util.command.buildPath(results[0].command, args.options, parent.clerk.indexer.index());
            execPath(path);
